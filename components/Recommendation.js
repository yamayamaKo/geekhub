...
const Recommend = (props) => {
    ...
    const [recommend, setRecommend] = useState(Array(13).fill(-1))
    ...
    for (let i=0; i<4; i++) if (recommend[i]!=-1) day_one_recommend.push(<EventCard session_id={recommend[i]} />)
    for (let i=4; i<8; i++) if (recommend[i]!=-1) day_two_recommend.push(<EventCard session_id={recommend[i]} />)
    for (let i=8; i<13; i++) if (recommend[i]!=-1) day_three_recommend.push(<EventCard session_id={recommend[i]} />)
